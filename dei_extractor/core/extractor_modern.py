#!/usr/bin/env python3
"""
DEI Modern PDF Invoice Data Extractor

This script extracts data from Greek DEI (Public Power Corporation) PDF invoices
using the modern layout format with 3-row block structure parsing.

Features:
- Extracts data from modern format PDF files
...
Version: 1.0 - Modern Dedicated Extractor
"""
...
# Financial patterns to ignore
FINANCIAL_PATTERNS = [
    re.compile(r"ΣΥΝΟΛΟ\s+ΠΛΗΡΩΤΕΟ", re.IGNORECASE),
    re.compile(r"ΠΡΟΗΓΟΥΜΕΝΟ\s+ΥΠΟΛΟΙΠΟ", re.IGNORECASE),
    re.compile(r"ΕΝΤΟΚΟΣ\s+ΥΠΟΛΟΙΠΟ", re.IGNORECASE),
    re.compile(r"ΠΕΡΙΟΔΟΣ\s+ΚΑΤΑΝΑΛΩΣΗΣ", re.IGNORECASE),
]
...
class DEIModernExtractor(LoggerMixin):
    """Dedicated extractor for DEI modern layout PDFs."""
    ...
    def parse_pdf(self, pdf_path: str) -> List[Dict]:
        """Parse a single modern PDF file and extract invoice records."""
        logger.info(f"Processing modern PDF: {pdf_path}")
    ...
    def process_files(self, file_paths: List[str]) -> pd.DataFrame:
        """Process multiple modern PDF files and return a DataFrame."""
        all_records = []
        ...
        if all_records:
            ...
            logger.info(f"Sorted {len(all_records)} modern records by ΑρΠαροχής")
        ...
        # Log summary
        logger.info(
            f"Processed {len(file_paths)} modern files, extracted {len(df)} records"
        )
    ...
    def write_outputs(self, df: pd.DataFrame, output_dir: str = "."):
        ...
        if df.empty:
            logger.warning("No modern data to write")
            return
        ...
        if "ΑρΠαροχής" in df_output.columns:
            ...
            logger.info(f"Sorted {len(df_output)} modern records by ΑρΠαροχής")

        if "ΑρΠαροχής" in fop_df.columns and not fop_df.empty:
            fop_df = fop_df.sort_values(by=["ΑρΠαροχής"])
            logger.info(f"Sorted {len(fop_df)} modern ΦΟΠ records by ΑρΠαροχής")

        if "ΑρΠαροχής" in epag_df.columns and not epag_df.empty:
            epag_df = epag_df.sort_values(by=["ΑρΠαροχής"])
            logger.info(
                f"Sorted {len(epag_df)} modern Επαγγελματικό records by ΑρΠαροχής"
            )

        # Write all records
        df_output.to_csv(
            output_path / "ολα_modern.csv", index=False, encoding="utf-8-sig"
        )
        df_output.to_excel(output_path / "ολα_modern.xlsx", index=False)

        # Write ΦΟΠ records
        if not fop_df.empty:
            fop_df.to_csv(
                output_path / "φoπ_modern.csv", index=False, encoding="utf-8-sig"
            )
            fop_df.to_excel(output_path / "φoπ_modern.xlsx", index=False)

        # Write Επαγγελματικό records
        if not epag_df.empty:
            epag_df.to_csv(
                output_path / "επαγγελματικα_modern.csv",
                index=False,
                encoding="utf-8-sig",
            )
            epag_df.to_excel(output_path / "επαγγελματικα_modern.xlsx", index=False)

        logger.info("Modern output files written successfully")

        # Write warnings to log
        if self.warnings:
            with open(output_path / "warnings_modern.log", "a", encoding="utf-8") as f:
                f.write(f"\n--- Modern Processing completed at {datetime.now()} ---\n")
